...
export default function FilterAppBar({ props, profileData, cardsData }) {
  ...
  const changeTag = (option) => {
    if (option.choice === null)
      return { value: option.value, id: option.id, choice: "assign" };
    if (option.choice === "assign")
      return { value: option.value, id: option.id, choice: "reject" };
    if (option.choice === "reject")
  ...
  const [allTagState, setAllTagState] = React.useState({
    value: undefined,
    id: undefined,
    choice: null,
  });
  ...
  const selectAllTags = (event, optionsTag) => {
    ...
    setAllTagState(
      changeTag({ value: undefined, id: undefined, choice: choice })
    );
    for (let index = 0; index < optionsTag.length; index++) {
      const newOptionTag = changeTag({
        value: optionsTag[index].value,
        id: optionsTag[index].id,
        choice: choice,
      });
      optionsTag[index] = newOptionTag;
    }
  ...
  useEffect(() => {
    const newDataFilter = JSON.parse(
      localStorage.getItem(`u${userID}dataFilter`)
    );
    ...
    if (newDataFilter !== null) {
      ...
      if (
        newDataFilter.filterTagsMethod === undefined ||
        newDataFilter.filterTagsMethod === null
      ) {
        setFilterTagsMethod(0);
      } else {
  ...
  const copyWids = () => {
    ...
    navigator.clipboard.writeText(wids).then(
      function () {
          ...
          <div className={classes.center}>
            <Button
              ...
              style={
                profileData
                  ? {
                      backgroundColor: changeUserColor(
                      ...
                      <MenuList id="split-button-menu">
                        {optionsSort.map((option, index) => (
                          <MenuItem
                            ...
                            selected={false}
                            onClick={(event) =>
                              handleMenuItemClickSort(event, index)
                            }
                            // style={getSortStyles(option)}
                          >
                            <a className={classes.tag_value}>{option.value}</a>
                            {option.choice === "assign" ? (
                              <ArrowDropUpIcon className={classes.icon} />
                            ) : option.choice === "reject" ? (
                              <ArrowDropDownIcon className={classes.icon} />
                            ) : (
                              <a style={{ marginLeft: 34 }}></a>
                            )}
                          </MenuItem>
                        ))}
                      </MenuList>
                ...
                >
                  <Paper>
                    <ClickAwayListener onClickAway={handleCloseTag}>
                      ...
                      >
                        {/* (o,oo)=>o.value.length-oo.value.length */}
                        {/* a.value.toLowerCase()==="ulubione" */}
                        <MenuItem
                        ...
                        >
                          <a
                            className={classes.tag_value}
                            style={{
                              borderBottom: "1px solid #5e636e",
                              textAlign: "center",
                            }}
                          >
                            Zaznacz wszystko
                        ...
                        {optionsTag.map((option, index) => (
                          <MenuItem
                            key={option.value}
                            onClick={(event) =>
                              handleMenuItemClickTag(event, index)
                            }
                            // style={getTagStyles(option)}
                          >
                            <a className={classes.tag_value}>
                              {option.value.toLowerCase() == "kosz" ? (
                                "🗑️"
