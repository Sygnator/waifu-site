...
const CardsDeck = (props) => {
  ...
  useEffect(async () => {
    ...
    if (cardsOnPage > 1) {
      await axios.post(`https://api.sanakan.pl/api/waifu/ultimate/cards/${(page-1) * cardsOnPage}/${cardsOnPage}`, localFilter)
      .then((res) => {
  ...
  const handleOpenCardDetails = (index) => () => {
    window.location.href=`#/cards/ultimate/${cardsData[index].id}`
  ...
  const handleIndexUp = () => {
    if (detailsIndex+1 != cardsData.length+1) {
      window.location.href=`#/cards/ultimate/${cardsData[detailsIndex+1].id}`
    }
