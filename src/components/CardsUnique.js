...
const CardsDeck = (props) => {
    ...
    const [localFilter, setLocalFilter] = useState(JSON.parse(localStorage.getItem(`uUniquefilter`)));
  ...
  useEffect(() => {
    let lUsername = JSON.parse(localStorage.getItem(`u204697username`))
    if (lUsername !== null && parseInt(lUsername.reqTime,10)+72000000 > new Date().getTime()) {
      setNick(lUsername.name)
    } else if(nick===undefined) {
      axios.get(`https://api.sanakan.pl/api/user/shinden/204697/username`).then((res) => {
        const newNick = res.data
        setNick(newNick)
        localStorage.setItem(`u204697username`, JSON.stringify({name: newNick, reqTime: new Date().getTime()}));
      }).catch((error) => {
    ...
    const filterUpdate = (filterData) => {
      localStorage.setItem(`u204697filter`, JSON.stringify(filterData))
      setLocalFilter(filterData)
      return JSON.parse(localStorage.getItem(`u204697filter`));
    };
  ...
  useEffect(async () => {
    let lProfile = JSON.parse(localStorage.getItem(`u204697profile`))
        ...
        await axios.get(`https://api.sanakan.pl/api/waifu/user/204697/profile`).then((res)=> {
            ...
            localStorage.setItem(`u204697profile`, JSON.stringify({profil: {...newProfileData, gallery: newProfileGallery}, reqTime: new Date().getTime()}));
        }).catch((error)=>{
          setStatus(404)
        })
    }
  ...
  useEffect(async () => {
    ...
    if(profileData!=undefined) {
        await axios.post(`https://api.sanakan.pl/api/waifu/unique/cards/${(page-1)*cardsOnPage}/${cardsOnPage}`, localFilter).then((res)=> {
                ...
                if (cardID) {
                  ...
                  if (indexCa >= 0) {
                    ...
                    window.location.href=`#/user/204697/cards`
                  }
                } 
  ...
  const clearData = () => {
    localStorage.removeItem(`u204697filter`)
    localStorage.removeItem(`u204697dataFilter`)
  ...
  const handleOpenCardDetails = (index) => () => {
    window.location.href=`#/user/204697/card/${cardsData[index].id}`
  ...
  const handleIndexUp = () => {
    if (detailsIndex+1 != cardsData.length+1) {
      window.location.href=`#/user/204697/card/${cardsData[detailsIndex+1].id}`
    }
